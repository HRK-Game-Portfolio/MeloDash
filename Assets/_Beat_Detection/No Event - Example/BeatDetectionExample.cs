...
public class BeatDetectionExample : MonoBehaviour {
    ...
    /********************************************************************************
	 ********************************************************************************
	 This is th only function you need to use. It catch the type of beat detected.
	 When using frequency mode you can get several types of beat at the same time.
	...
	********************************************************************************/
    void Update() {
		...
		*/
         ...
         * When a yield statement is used, the coroutine will pause execution and automatically resume
         * at the next frame Coroutines are excellent when modeling behavior over several frames.
         * Coroutines have virtually no performance overhead. StartCoroutine function always returns immediately,
         * however you can yield the result. Yielding waits until the coroutine has finished execution.
         * There is no guarantee that coroutines end in the same order that they were started,
         * even if they finish in the same frame.
         */
        if ((theBeat & (int)beatType.kick) != (int)beatType.None) {
